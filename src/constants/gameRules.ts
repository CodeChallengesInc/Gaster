export const LONE_ANT_RULES = `# Rules

Lone ant is a challenge where the player controls a single ant. Ants search the provided grid for food.

## Ant Abilities

Each submission represents a single ant that will attempt to gather food on the 250x100 grid (grid size may vary as it's configurable on the backend, but the default is 250x100). Ants have the following abilities:
...
An ant function may only use a pseudo random number generator that it supplies itself, which doesn't store state. Due to this, Math.random() is explicitly forbidden, as it relies on storing state to generate the next random number. 
...
\`\`\`
`;
export const SPAWNING_ANT_RULES = `# Rules

Spawning Ants is a challenge where the player controls a single ant. Ants search the provided grid for food.
...
{cell0, color:9}: color must be from 1-8.
\`\`\``;
export const FORMIC_ANT_RULES = `# Rules
